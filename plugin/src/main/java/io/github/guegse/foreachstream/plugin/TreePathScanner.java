...
import static io.github.guegse.foreachstream.plugin.ASTHelpers.getSymbol;

public class TreePathScanner extends com.sun.source.util.TreePathScanner<Void, Void> {
    ...
    private JCTree.JCBlock staticBlock;

    public TreePathScanner(TreeMaker treeMaker, Names names, Trees trees, Map<String, String> availableMethodsToTheirClasses, CompilationUnitTree compilationUnit, Context context) {
        this.trees = trees;
    ...
    public Void visitClass(ClassTree node, Void o) {
        ...
        staticBlock = treeMaker.Block(0, List.nil());
        classDecl.defs = classDecl.defs.prepend(staticBlock);
            ...
            if (isTerminalMethod(methodName)) {
                ...
                if (canReplace) {
                    ...
                    for(JCTree.JCExpression arg : arguments) {
                        ...
                        JCTree.JCExpression newArg = treeMaker.Literal(TypeTag.BOT, null);
                        args = args.append(newArg);
                    }
                    ...
                    // Add the new method invocation to the static block
                    JCTree.JCMethodInvocation newMethodInvocation = treeMaker.Apply(
                    ...
                    // Add the new method invocation to the static block
                    staticBlock.stats = staticBlock.stats.append(treeMaker.Exec(newMethodInvocation));
                }
            }
        }
        return super.visitMethodInvocation(node, o);
